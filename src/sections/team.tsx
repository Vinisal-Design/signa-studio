...
// TODO(operator): swap initials for real LinkedIn photos via <Image> when ready.
// Slot the file under public/team/<slug>.webp and pass via founder.photo.
...
export function Team() {
  return (
    ...
    >
      ...
      <div
        className="glow-warm pointer-events-none absolute right-[-15%] top-[40%] h-[50vh] w-[50vh]"
        aria-hidden="true"
      />
      ...
      <div className="relative z-10 mx-auto max-w-6xl">
        ...
        <FadeIn className="text-center">
          <span className="eyebrow-mono text-text-muted">03 · Quem constrói</span>
          <h2
            id="team-heading"
          ...
          <p className="mx-auto mt-5 max-w-2xl text-[1rem] leading-[1.7] text-text-muted">
            ...
            <span className="text-foreground">
              Quem responde no WhatsApp é quem está construindo o seu site.
            </span>
          </p>
          <p className="mx-auto mt-6 max-w-xl text-[0.95rem] italic leading-[1.65] text-text-dim">
          ...
          <ul className="grid list-none gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {founders.map((founder) => (
              <StaggerItem key={founder.name}>
                <li>
                  <article className="card-glow group relative flex h-full flex-col rounded-2xl border border-white/[0.06] bg-white/[0.02] p-7 transition-[border-color,background-color] duration-500 ease-out hover:border-white/[0.12] hover:bg-white/[0.04]">
                    <div
                    ...
                    <ul className="mt-6 flex list-none flex-wrap justify-center gap-1.5">
                    ...
                    {founder.linkedin ? (
                      <a
                        ...
                        className="mt-6 inline-flex items-center justify-center gap-1.5 text-[11px] font-medium text-text-dim transition-colors duration-300 hover:text-accent-light focus-visible:outline-none focus-visible:text-accent-light"
                      ...
                      // Spacer keeps card foot aligned across the row when one
                      // founder lacks a public LinkedIn link. Same vertical
                      // footprint as the actual link (~28px including margin).
                      <span className="mt-6 block h-[18px]" aria-hidden="true" />
                    )}
                  </article>
                </li>
              </StaggerItem>
        ...
        <StaggerContainer className="mt-16" stagger={0.08}>
          <ul className="grid list-none gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {principles.map(({ Icon, title, description }) => (
              <StaggerItem key={title}>
                <li className="card-glow group h-full rounded-2xl border border-white/[0.04] bg-white/[0.02] p-6 transition-[border-color,background-color] duration-500 ease-out hover:border-white/[0.08] hover:bg-white/[0.04]">
                  <div className="mb-5 inline-flex h-11 w-11 items-center justify-center rounded-xl border border-white/[0.06] bg-gradient-to-br from-accent-light/10 to-[#f4c89a]/[0.06] text-accent-light transition-[transform,border-color] duration-500 ease-out group-hover:scale-105 group-hover:border-white/[0.12]">
        ...
        <FadeIn className="mt-16">
          <p className="mx-auto max-w-2xl text-center text-[0.95rem] italic leading-[1.7] text-text-muted">
          ...
          <p className="mx-auto mt-6 max-w-xl text-center text-[0.95rem] leading-[1.65] text-text-muted">
            ...
            <span className="text-text-dim">
              Quem entra, entra com nome e WhatsApp dos quatro.
            </span>
          </p>
        </FadeIn>
        ...
        <FadeIn className="mt-12">
          <div className="mx-auto max-w-3xl rounded-2xl border border-white/[0.04] bg-white/[0.02] p-8 text-center backdrop-blur-sm">
            <p className="mb-4 text-[11px] font-semibold uppercase tracking-[0.2em] text-accent-light">
